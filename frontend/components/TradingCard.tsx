...
export default function TradingCard({ marketId }: TradingCardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* REAL STATS PANEL — EXACTLY LIKE YOUR SCREENSHOT */}
      {amount && parseFloat(amount) > 0 && (
        <div className="bg-gray-50 rounded-lg p-4 space-y-3">
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">Price change</span>
            <span className="font-bold">${currentPrice.toFixed(2)} → ${newPrice.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm">
      ...
      <button
        onClick={handleTrade}
        disabled={isPending || isConfirming || isApproving || isApprovalConfirming || !amount || parseFloat(amount) <= 0}
